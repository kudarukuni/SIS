			...
			<form action="student_signup.php" method="POST" class="form-signin" onsubmit="checkfields()" id="signin_student">
			...
			<button id="signin" name="submit" class="btn btn-info" type="submit"><i class="icon-check icon-large"></i> Sign Up </button>
			</form>
			...
			jQuery("#signin_student").submit(function(e){
				...
				$.ajax({
					...
					if(html=='true')
					{
					$.jGrowl("Welcome to CUZ E-Learning Management System", { header: 'Sign up Success' });
					var delay = 2000;
						setTimeout(function(){ window.location = 'dashboard_student.php'  }, delay);  
					}else if(html=='false'){
						$.jGrowl("student was not found in the database Please makesure to Check Your ID Number or Firstname, Lastname and the Section You Belong. ", { header: 'Sign Up Failed' });
					}
					}
				});			
				}
					...
					{
					$.jGrowl("Student was not found in the database", { header: 'Sign Up Failed' });
					}
				});
			});*/
			...
			{
				if (document.getElementById("username").value == "" || document.getElementById("firstname").value == "" || document.getElementById("lastname").value == "" || document.getElementById("password").value == "" || document.getElementById("cpassword").value == "" || document.getElementById("class_id").value == "")
				{
